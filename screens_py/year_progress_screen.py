...
class YearProgress(Screen):
    def year_summary(self):
        dt = datetime.today()
        year = dt.date() - timedelta(days=365)
        print(year)

        mycursor = main.sqliteConnection.cursor()
        mycursor.execute(f"SELECT count(id_task) "
                         f"FROM work_plan "
                         f"WHERE done_date > ? AND done_date <= date('now') ", (year,))
        row = mycursor.fetchone()
        done_tasks_during_year = row[0]
        print(done_tasks_during_year)

        self.ids.year_stats.add_widget(
            MDRoundFlatButton(
                text=str(done_tasks_during_year) + "\n\nD O N E\nT A S K S",
        ...
        mycursor.execute(f"SELECT count(id_task) "
                         f"FROM work_plan "
                         f"WHERE scheduled_date > ? AND scheduled_date <= date('now') ", (year,))
        row = mycursor.fetchone()
        all_scheduled_tasks = row[0]
        if all_scheduled_tasks == None:
            precent = 'PRO8LEM'
        else:
            precent = int(done_tasks_during_year / all_scheduled_tasks * 100)
        print(precent)
        self.ids.year_stats.add_widget(
            MDRoundFlatButton(
                text=str(precent) + "%\n\nD O N E\nT A S K S",
        ...
        mycursor.execute("SELECT sum(execution_time) "
                         "FROM work_plan "
                         "WHERE done_date <= date('now') AND done_date > ? ", (year,))
    ...
    def year_analysis(self):
        dt = datetime.today()
        year = dt.date() - timedelta(days=30)
        print(year)
        mycursor = main.sqliteConnection.cursor()
        mycursor.execute(
        ...
        for i in range(len(task_name)):
            ...
            self.ids.year_progress.add_widget(
                ThreeLineListItem(
